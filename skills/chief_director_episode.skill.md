...
### Pax版提示词格式（必须传递给下游）
- **Image_Prompt（中文）**：画面提示词卡（字段化、多行文本），字段顺序固定（镜号/剧本内容/画面描述/叙事功能/时空坐标/摄影参数/氛围/音效等）。
- **Video_Prompt（中文）**：三段式：
  1) 镜头运动：...
  2) 画面描述：...
  3) 声音：...
- 声音默认策略：**无人声、无对白、无背景音乐（VO=0）**；如需环境音，只允许自然环境音。
- 若台词为空（- / 无），Video_Prompt 不得写“正在说话”，只能写口型变化/无声呢喃等可拍表现。
